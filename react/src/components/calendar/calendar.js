...
Calendar = function (
  ...
  this.displayStatusBars = displayStatusBars
...
export function setup(params) {
  // Calendar.setup11 = function (params) {
  ...
  param_default("displayStatusBars", true)
  ...
  // console.log(params)
  if (
    !(params.flat || params.inputField || params.displayArea || params.button)
  ) {
    // console.log(params)
    // alert("Calendar.setup:\n  Nothing to setup (no fields found).  Please check your code");
    sysUtil.showErrorMessage(
      "Calendar.setup:\n  Nothing to setup (no fields found).  Please check your code"
    )
    return false
  }
  ...
  function onSelect(cal) {
    if (cal.params.flat) {
      if (typeof cal.params.flatCallback == "function") {
        ...
        sysUtil.showErrorMessage("No flatCallback given -- doing nothing.")
      }
      return false
    }
...
Calendar.tableMouseOver = function (ev) {
  ...
  if (el.navtype == 50 && target != el) {
    ...
    while (count-- > 0)
      if (decrease) {
        if (!(--i in range)) i = range.length - 1
...
Calendar.dayMouseDown = function (ev) {
  ...
  if (el.navtype != 300) {
    // with (Calendar) {
    if (el.navtype == 50) el._current = el.firstChild.data
    Calendar.addClass(el, "hilite active")
...
Calendar.dayMouseOver = function (ev) {
  ...
  if (el.ttip) {
    if (el.ttip.substr(0, 1) == "_") {
      ...
      date = new Date(Calendar.getFullYear(), Calendar.getMonth(), el.caldate)
      // }
...
Calendar.prototype.create = function (_par) {
  ...
  hh(Calendar._TT["TODAY"], this.weekNumbers ? 4 : 3, 0)
  ...
  if (this.showsTime) {
    ...
    ;(function () {
      ...
      cal.onUpdateTime = function () {
        ...
        if (t12) {
          if (/pm/i.test(AP.firstChild.data) && h < 12) h += 12
  ...
  if (this.displayStatusBars) {
    row.className = "footrow"
  } else {
    row.className = "nofootrow"
  }
  ...
  this.tooltips = cell
  // this.tooltips.visibility="hidden";
...
Calendar.prototype.showAtElement = function (el, opts) {
  ...
  if (Calendar.is_khtml)
    ...
    // setTimeout("Calendar.continuation_for_the_fucking_khtml_browser1()", 10);
...
Calendar.prototype.hideShowCovered = function () {
    ...
    function getVisib(obj) {
      ...
      if (!value) {
        if (
          ...
          // Gecko, W3C
          if (!Calendar.is_khtml)
            value = document.defaultView.document.defaultView
  ...
  if (Calendar.is_khtml)
    // setTimeout("Calendar.continuation_for_the_fucking_khtml_browser()", 10);
    // window.calendar.setTimeout("Calendar.continuation_for_the_fucking_khtml_browser2()",10)
    // setTimeout(Calendar.continuation_for_the_fucking_khtml_browser2(), 10);
    setTimeout(function () {
      Calendar.continuation_for_the_fucking_khtml_browser()
    }, 10)
  else Calendar.continuation_for_the_fucking_khtml_browser()
