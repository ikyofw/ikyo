...
export function isActive(active: Types.StoreState["active"], point: Point.Point): boolean {
...
export function writeTextToClipboard(event: ClipboardEvent, data: string): void {
...
export function isFormulaCell(cell: Types.CellBase): cell is Types.CellBase<string> {
  return Formula.isFormulaValue(cell.value)
}

/** Normalize given selected range to given data matrix */
export function normalizeSelected(selected: PointRange.PointRange | null, data: Matrix.Matrix<unknown>): PointRange.PointRange | null {
...
export function getSelectedCSV(selected: PointRange.PointRange | null, data: Matrix.Matrix<Types.CellBase>): string {
  if (!selected) {
    return ""
  }
 ...
 */
export function calculateSpreadsheetSize(data: Matrix.Matrix<unknown>, rowLabels?: string[], columnLabels?: string[]): Matrix.Size {
...
export function convertPointMapToPointSet(map: PointMap.PointMap<unknown>): PointSet.PointSet {
  return PointMap.map(() => true, map)
}

/** Get the range of copied cells. If none are copied return null */
export function getCopiedRange(copied: Types.StoreState["copied"], hasPasted: boolean): PointRange.PointRange | null {
...
export function transformCoordToPoint(coord: { row: { index: number }; column: { index: number } }): Point.Point {
  return { row: coord.row.index, column: coord.column.index }
}

/** Should spreadsheet handle clipboard event */
export function shouldHandleClipboardEvent(root: Element | null, mode: Types.Mode): boolean {
