...
import { getNextRow, getNextColumn } from "./reducer"
...
export function toRange(selection: Selection, data: Matrix.Matrix<unknown>): PointRange.PointRange | null {
  ...
  switch (selection.type) {
    case EntireType.Row: {
      ...
      return PointRange.create({ row: selection.start, column: 0 }, { row: selection.end, column: max.column })
    }
    case EntireType.Column: {
      const max = Matrix.maxPoint(data)
      return PointRange.create({ row: 0, column: selection.start }, { row: max.row, column: selection.end })
    }
...
export function modifyEdge(state: Types.StoreState, edge: Direction): Selection {
  if (state.active && state.selected) {
    if (PointRange.is(state.selected)) {
      return modifyRangeEdge(state, edge)
    }
    switch (state.selected.type) {
      case EntireType.Row: {
        return modifyEntireRowsEdge(state.selected, state.active, state.data, edge)
      }
      case EntireType.Column: {
        return modifyEntireColumnsEdge(state.selected, state.active, state.data, edge)
      }
    }
  }
...
export function modifyRangeEdge(state: Types.StoreState, edge: Direction): PointRange.PointRange {
  const range = state.selected as PointRange.PointRange
  ...
  let nextField = range[keyToModify][field]
  if (field === "column") {
    // Get the column number of the new range's start or end in the visible columns.
    const nextColumn = getNextColumn(state, range[keyToModify][field], delta) 
    if (nextColumn > 1) {
      // When reaching the first column of visible columns, prevent further left movement.
      nextField = nextColumn
    }
  } else if (field === "row") {
    // Get the row number of the new range's start or end in the visible row.
    const nextRow = getNextRow(state, range[keyToModify][field], delta) 
    const maxRow = state.showRange.slice(-1)[0]
    const minRow = state.showRange[0]
    if (nextRow <= maxRow && nextRow >= minRow) {
      // When reaching the upper or lower boundary of the visible rows, prevent moving up or down.
      nextField = nextRow 
    }
  }
  ...
  return normalizeRange(nextRange, state.data)
}
