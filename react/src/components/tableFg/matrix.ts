...
export function createEmpty<T>(rows: number, columns: number): Matrix<T> {
  ...
  for (let i = 0; i < rows; i++) {
    matrix[i] = Array(columns).fill({ value: "" })
  }
  return matrix
}
...
export function slice<T>(startPoint: Point.Point, endPoint: Point.Point, matrix: Matrix<T>): Matrix<T> {
...
export function set<T>(point: Point.Point, value: T, matrix: Matrix<T>): Matrix<T> {
...
export function mutableSet<T>(point: Point.Point, value: T, matrix: Matrix<T>): void {
...
export function map<T, T2>(func: (value: T | undefined, point: Point.Point) => T2, matrix: Matrix<T>): Matrix<T2> {
 ...
 */
export function join(matrix: Matrix<unknown>, horizontalSeparator = "\t", verticalSeparator = "\n"): string {
  let joined = ""
...
export function toArray<T1, T2>(matrix: Matrix<T1>, transform: (cell: T1 | undefined, coords: Point.Point) => T2): T2[]
...
export function toArray<T1, T2>(matrix: Matrix<T1>, transform?: (cell: T1 | undefined, coords: Point.Point) => T2) {
  const array = []
