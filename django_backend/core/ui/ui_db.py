...
import core.utils.model_utils as ikModelUtils
...
from iktools import IkConfig, getAppNames
...
def createCSVFileWithDatabase(select_screen_sn: str = None):
    try:
        if isNotNullBlank(select_screen_sn):
            screen_sns = [{'screen_sn': select_screen_sn}]
        else:
            screen_sns = Screen.objects.values('screen_sn').distinct().order_by('screen_sn')
            logger.info('Export all screen definition to csv file start ...')
        for screen_sn_dict in screen_sns:
            screen_sn = screen_sn_dict['screen_sn']
            latest_screen = Screen.objects.filter(screen_sn__iexact=screen_sn).order_by('-rev').first()

            if isNotNullBlank(latest_screen):
                app_nms = getAppNames()
                if latest_screen.app_nm not in app_nms:
                    continue

                filename = '%s.csv' % latest_screen.screen_sn
                output_file = Path(os.path.join(latest_screen.app_nm, ikui.SCREEN_RESOURCE_FOLDER_PATH, 'csv', filename))

                if isNotNullBlank(select_screen_sn):
                    logger.info('Export screen definition [%s] to csv file [%s] start ...' % (select_screen_sn, output_file))
                ...
                try:
                    exp_data = __getExpDataFromDB(latest_screen, isCSV=True)
                except Exception as exc:
                    logger.error('Get screen=%s (Rev %s) data from database failed: %s' % (latest_screen.screen_sn, latest_screen.rev, str(exc)))
                ...
                csv.Write2CsvFile(output_file, exp_data, comments="This is a comment line.", sort_keys=True)
                if isNotNullBlank(select_screen_sn):
                    logger.info('Export screen definition [%s] to csv file [%s] success.' % (select_screen_sn, output_file))

        logger.info('Export all screen definition to csv file success.')
        return Boolean2(True, 'Create CSV File Success.')
    except Exception as e:
        logger.error(e, exc_info=True)
        return Boolean2(False, 'Create CSV File error! Screen ID: [%s]' % select_screen_sn if isNotNullBlank(select_screen_sn) else 'All Screens')
...
def screenDbWriteToExcel(screenRc: Screen, fileRmk: str = None) -> Boolean2:
    ...
    if isNotNullBlank(fileRmk):
        ...
        d = createCSVFileWithDatabase(select_screen_sn=screenRc.screen_sn)
        if not d.value:
            return d
    ...
    def spreadsheetWriter(expData, template_file, outputFile):
        ...
        b = sw.write()
        if not b.value:
            raise IkException(str(b.dataStr))  # raise an Exception, not a string

        logger.info('Export screen definition [%s] to excel file [%s] success', screenRc.screen_sn, outputFile)

        screenFileRc = ScreenFile.objects.filter(screen=screenRc).order_by('-file_dt').first()
        if not screenFileRc:
            screenFileRc = ScreenFile(screen=screenRc, file_nm=filename, file_path=screenRc.app_nm, file_dt=datetime_.now())
        ...
        c = ptrn.save()
        if not c.value:
            raise IkException(str(c.dataStr))

    expData = __getExpDataFromDB(screenRc)
    spreadsheetWriter(expData, template_file, outputFile)
