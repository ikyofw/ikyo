...
from core.core.http import get_ip, is_support_session
...
def __getExcludedMenus():
    querySet = Menu.objects.filter(Q(menu_nm__icontains='HOME') | Q(menu_nm__iexact='MENU') | Q(menu_nm__icontains='Inbox'))
    data = []
    for i in querySet:
