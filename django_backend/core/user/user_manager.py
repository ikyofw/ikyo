...
from core.view.auth_view import getCurrentView
...
def getUserDefaultOffice(usrId) -> str | None:
    with connection.cursor() as cursor:
        cursor.execute('SELECT o.code FROM ik_usr_office uo, ik_office o WHERE uo.office_id=o.id AND uo.id=%s ORDER BY uo.seq LIMIT 1' % usrId)
        rs = dbUtils.dictfetchall(cursor)
    return None if dbUtils.isEmpty(rs) else rs[0]['code']


def getUserDefaultOfficeID(usrId) -> int | None:
    with connection.cursor() as cursor:
        cursor.execute('SELECT o.id FROM ik_usr_office uo, ik_office o WHERE uo.office_id=o.id AND uo.id=%s ORDER BY uo.seq LIMIT 1' % usrId)
        rs = dbUtils.dictfetchall(cursor)
    return None if dbUtils.isEmpty(rs) else rs[0]['id']
