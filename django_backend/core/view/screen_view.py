...
from core.sys.access_log import addAccessLog
...
PARAMETER_KEY_NAME_LAST_SEARCH_DATA = 'LAST_SEARCH_DATA'
...
class ScreenAPIView(AuthAPIView):
    ...
    def _getScreenResponse(self) -> ikhttp.IkSccJsonResponse:
        ...
        try:
            ...
            if self.beforeDisplayAdapter:
                if screen.subScreenName != ikui.MAIN_SCREEN_NAME:  # clear screen layout when show dialog
                    screen.layoutParams = ''
            ...
            return ikhttp.IkErrJsonResponse(message='System error: %s' % str(e))
        finally:
    ...
    def __updateDataCursor(self, fieldGroup, data) -> None:
        if data is not None and fieldGroup.groupType == ikui.SCREEN_FIELD_TYPE_RESULT_TABLE and fieldGroup.editable:
            # 1. check update from client. E.g. user click the Edit button
            cursorName = None
            cursorValue = None
            ...
            if isNotNullBlank(cursorValue):
                if isinstance(data, Model):
                    ...
                    for rc in data:
                        ...
                        elif isinstance(rc, dict) and rc.get('id') == cursorValue:  # for dict table data set cursor, YL, 2025-07-29
                            rc[ikDbModels.MODEL_RECORD_DATA_CURRENT_KEY_NAME] = True
    ...
    def get_last_static_revision_file(self, filename=None, code=None, return_full_path: bool = False) -> str:
        ''' 
        ...
        def get_path(c):
            ...
            if c is not None:
                ...
                p = p / str(self.__class__.__name__)
            return p
        ...
        try:
            ...
            if self.__isCallViewAPI:
                ...
                try:
                    ...
                    logger.debug('%s Object [%s] cannot convert to json string.' % (className, self.getRequestData()))
    ...
    def getQueryParams(self) -> QueryDict:
    ...
    def read_template_file_with_prams(self, filename, code=None, pram=None) -> str:
        ...
        return template_utils.loadTemplateFile(file_path, pram)
    ...
    def __getRecordSetData(self, fieldGroup, field, recordsetName) -> object:
        ...
        if rcs is not None:
            ...
            if not isNullBlank(currentRecordID):
                model_utils.locateToFirstByID(rcs, int(currentRecordID))
        if field is None and fieldGroup.isDetail():
            return rcs[0] if isNotNullBlank(rcs) and len(rcs) > 0 else None
        return rcs
    ...
    def _BIFCancel(self, fieldGropName: str = None) -> ikhttp.IkJsonResponse:
        ...
        if isNullBlank(fieldGropName):
            ...
            fieldGropName = fieldGropNames[0]
        screen = self._screen
        fg = screen.getFieldGroup(fieldGropName)
        if fg is None:
            return ikhttp.IkErrJsonResponse(message='Fieldgroup [%s] is not found for build-in function [new].' % fieldGropName)
