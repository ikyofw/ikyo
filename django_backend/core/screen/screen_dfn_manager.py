...
import core.ui.ui_cache as ikuiCache
...
def saveScreen(self, screenSn, isNew, currentBtnClick) -> Boolean2:
    try:
        ...
        screenDtlFg: Screen
        if screenDtlFg:
            sn = str_utils.stripStr(screenDtlFg.screen_sn)
            if str_utils.isEmpty(sn):
                return Boolean2(False, "Screen ID is mandatory, please check.")

            screenTitle = str_utils.stripStr(screenDtlFg.screen_title)
            if str_utils.isEmpty(screenTitle):
                return Boolean2(False, "Screen Title is mandatory, please check.")

            layoutType = str_utils.stripStr(screenDtlFg.layout_type)
            if str_utils.isEmpty(layoutType):
                return Boolean2(False, "Screen Layout Type is mandatory, please check.")

            appNm = str_utils.stripStr(screenDtlFg.app_nm)
            if str_utils.isEmpty(appNm):
                return Boolean2(False, "Screen App Name is mandatory, please check.")

            classNm = str_utils.stripStr(screenDtlFg.class_nm)
            if isNotNullBlank(classNm) and not classNm.startswith(appNm):
                return Boolean2(False, "Screen Class Name must contain App Name, please check.")
            if isNotNullBlank(classNm) and not classNm.endswith(sn):
            ...
            if not str_utils.isEmpty(screenSn) and not isNew:  # update
            ...
            if isNew and str_utils.isEmpty(screenSn):
                screenSn = sn
            # check real modified
            # TODO
        ...
        if recordsetLists:
            ...
            for rc in recordsetLists:
                ...
                if rc.ik_is_status_delete():
                    ...
                    recordsetNm = str_utils.stripStr(rc.recordset_nm)
                    if str_utils.isEmpty(recordsetNm):
                        return Boolean2(False, "Recordset Name is mandatory, please check.")
                    if str_utils.isEmpty(rc.sql_fields):
                        rc.sql_fields = "*"
                    if str_utils.isEmpty(rc.sql_models):
...
def saveFieldGroup(self, screenSn, isNew, currentBtnClick) -> Boolean2:
    try:
        ...
        if fieldGroupDtlFg:
            ...
            if str_utils.isEmpty(fieldGroupDtlFg.seq):
                fieldGroupDtlFg.seq = sys.maxsize
            # validate field group
            fgName = str_utils.stripStr(fieldGroupDtlFg.fg_nm)
            if str_utils.isEmpty(fgName):
            ...
            if str_utils.isEmpty(fgTypeId):
                return Boolean2(False, "Group Type is mandatory, please check.")

            # validate page type & page size
            if not str_utils.isEmpty(fieldGroupDtlFg.data_page_type) and str_utils.isEmpty(fieldGroupDtlFg.data_page_size):
                return Boolean2(False, "Page Size is mandatory, please check.")
            if str_utils.isEmpty(fieldGroupDtlFg.data_page_type) and not str_utils.isEmpty(fieldGroupDtlFg.data_page_size):
            ...
            if fieldListFg:
                ...
                for rc in fieldListFg:
                    if not rc.ik_is_status_delete():
                        ...
                        rc.seq = str_utils.stripStr(rc.seq)
                        rc.field_nm = str_utils.stripStr(rc.field_nm)
...
def saveSubScreen(self, screenSn, currentBtnClick) -> Boolean2:
    try:
        screenRc = Screen.objects.filter(screen_sn__iexact=screenSn).order_by("-rev").first()
        if screenRc is None:
            return Boolean2(False, screenSn + " does not exist, please ask administrator to check.")
...
def saveFgLink(self, screenSn, isNew, currentBtnClick) -> Boolean2:
    try:
        ...
        if fgLinkDtlFg:
            ...
            # validate field group
            fgId = str_utils.stripStr(fgLinkDtlFg.field_group_id)
            if str_utils.isEmpty(fgId):
                return Boolean2(False, "Field Group Name is mandatory, please check.")
            pFgId = str_utils.stripStr(fgLinkDtlFg.parent_field_group_id)
            if str_utils.isEmpty(pFgId):
            ...
            parentKey = str_utils.stripStr(fgLinkDtlFg.parent_key)
            if str_utils.isEmpty(parentKey):
                return Boolean2(False, "Parent Key is mandatory, please check.")
            localKey = str_utils.stripStr(fgLinkDtlFg.local_key)
            if str_utils.isEmpty(localKey):
...
def saveFgHeaderFooter(self, screenSn, isNew, currentBtnClick) -> Boolean2:
    try:
        ...
        # validate field group
        fgId = str_utils.stripStr(fgHeaderFooterDtlFg.field_group_id)
        if str_utils.isEmpty(fgId):
            return Boolean2(False, "Field Group Name is mandatory, please check.")
        fId = str_utils.stripStr(fgHeaderFooterDtlFg.field_id)
        if str_utils.isEmpty(fId):
            return Boolean2(False, "Field Name is mandatory, please check.")
        headerLevel1 = str_utils.stripStr(fgHeaderFooterDtlFg.header_level1)
        # XH 2023-04-25 START
        # if strUtils.isEmpty(headerLevel1):
        #     return Boolean2(False, "Header level 1 is mandatory, please check.")
        headerLevel2 = str_utils.stripStr(fgHeaderFooterDtlFg.header_level2)
        # if strUtils.isEmpty(headerLevel2):
        #     return Boolean2(False, "Header level 2 is mandatory, please check.")
        headerLevel3 = str_utils.stripStr(fgHeaderFooterDtlFg.header_level3)
        # if strUtils.isEmpty(headerLevel3):
        #     return Boolean2(False, "Header level 3 is mandatory, please check.")
        footer = str_utils.stripStr(fgHeaderFooterDtlFg.footer)
...
def __createNewRevScreen(self,
    ...
    try:
        ...
        if not modifyFlag:
            return Boolean2(False, 'No changes detected. Nothing to save.')
        ...
        if not isNewScreen:  # just new screen
            ...
            for rc in recordsetRcs:
                if not rc.ik_is_status_delete():
                    ...
                    if not str_utils.isEmpty(orgRecordsetNm):
                        recordsetNewIdMap.update({orgRecordsetNm: rc.id})
            ...
            newCurrentFgId = None
            headerFooterDeleteIDs = []  # delete header and footer table id
            curSeq = 100
            for fgRc in lastRevFieldGroupRcs:
                if fgRc.id == currentFgId and isDeleteFieldGroup:  # delete.
                    continue
                if not str_utils.isEmpty(currentFgId) and fieldGroupRc and fgRc.id == currentFgId and not isDeleteFieldGroup:  # modify current field group info
                    ...
                    if fieldRcs:
                        ...
                        seq1 = 1
                        for fRc in fieldRcs:
                            if fRc.ik_is_status_delete():
                                headerFooterRc = ScreenFgHeaderFooter.objects.filter(screen=lastRevScreenRc, field=fRc).first()
                                if isNotNullBlank(headerFooterRc):
                                    headerFooterDeleteIDs.append(headerFooterRc.id)
                            else:
                                ...
                                if not str_utils.isEmpty(orgFieldNm):
                                    fgFieldNewIdMap.update({fRc.field_group.fg_nm + "-" + orgFieldNm: fRc.id})
            ...
            if fieldGroupRc and str_utils.isEmpty(currentFgId) and not isDeleteFieldGroup:
                fieldGroupRc.ik_set_status_new()
                fieldGroupRc.id = ScreenFieldGroup().assignPrimaryID()
            ...
            for rc in lastRevFgLinkRcs:
                if rc.id == currentFgLinkId and isDeleteFgLink or rc.field_group_id == currentFgId and isDeleteFieldGroup:
                    continue
                if not str_utils.isEmpty(currentFgLinkId) and fgLinkRc and rc.id == currentFgLinkId and not isDeleteFgLink:  # modify current field group link info
            ...
            # XH 2023-04-25 START
            # create new field group
            if fgLinkRc and str_utils.isEmpty(currentFgLinkId) and not isDeleteFgLink:
                fgLinkRc.ik_set_status_new()
                fgLinkRc.id = ScreenFgLink().assignPrimaryID()
                newCurrentFgLinkId = fgLinkRc.id  # for get new current field group link id
                # _import: use new screenRc and field group
            ...
            for rc in lastRevFgHeaderFooterRcs:
                ...
                if rc.id == currentFgHeaderFooterId and isDeleteFgHeaderFooter or rc.field_group_id == currentFgId and isDeleteFieldGroup:
                    continue
                if len(headerFooterDeleteIDs) > 0 and rc.id in headerFooterDeleteIDs:  # delete header footer table
                    continue
                if not str_utils.isEmpty(currentFgHeaderFooterId
            ...
            # XH 2023-04-25 START
            if fgHeaderFooterRc and str_utils.isEmpty(currentFgHeaderFooterId) and not isDeleteFgHeaderFooter:
                fgHeaderFooterRc.ik_set_status_new()
                fgHeaderFooterRc.id = ScreenFgHeaderFooter().assignPrimaryID()
                newCurrentFgHeaderFooterId = fgHeaderFooterRc.id  # for get new current header and footer table id
                # _import: use new screenRc and field group and field
        ...
        if b.value:
            ...
            if isNewScreen:
                ...
                viewClass = model_utils.get_model_class_2(screenRc.app_nm, screenRc.screen_sn, screenRc.class_nm)
...
def __is_model_class_exists(app_mm, screen_sn, class_nm):
    if not model_utils.is_model_class_exists(app_mm, screen_sn, class_nm):
...
def __is_excel_open(screenRc: Screen) -> Boolean2:
    ...
    try:
        ...
        return Boolean2(False, "File [%s] is already open. Please close it and try again." % filename)
