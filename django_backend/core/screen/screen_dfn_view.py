...
from core.screen import screen_dfn_manager
...
class ScreenDfnView(ScreenAPIView):
    ...
    def __init__(self) -> None:
        ...
        def beforeDisplayAdapter(screen: ikui.Screen):
            ...
            if screen.subScreenName == 'widgetPramsDialog':
                ...
                screen.setFieldsVisible(fieldGroupName='dialogWidgetPramsFg',
                                        fieldNames=[
                                            ...
                                            'recordsetField', 'dataUrlField', 'valueField', 'displayField', 'onChangeField', 'iconField', 'typeField', 'serverFilterField',
                                            'pageSizeField', 'columnField', 'titleField'
                                        ],
                                        visible=False)
                screen.setFieldGroupsVisible(fieldGroupNames='dialogWidgetDialogPramsFg', visible=False)
                if widgetNm in screen_dfn_manager.NO_PARAMETERS_WIDGET:
                    ...
                    screen.setFieldsVisible(fieldGroupName='dialogWidgetPramsFg',
                                            ...
                                            visible=True)
                elif widgetNm == ikui.SCREEN_FIELD_WIDGET_LINK:
                    screen.setFieldsVisible(fieldGroupName='dialogWidgetPramsFg', fieldNames=['valueField', 'displayField', 'titleField'], visible=True)
                ...
                elif widgetNm == ikui.SCREEN_FIELD_WIDGET_COMBO_BOX or widgetNm == ikui.SCREEN_FIELD_WIDGET_LIST_BOX:
                    screen.setFieldsVisible(fieldGroupName='dialogWidgetPramsFg',
                                            fieldNames=['dataField', 'recordsetField', 'dataUrlField', 'valueField', 'displayField', 'onChangeField'],
                                            visible=True)
                elif widgetNm == ikui.SCREEN_FIELD_WIDGET_ADVANCED_COMBOBOX:
                    screen.setFieldsVisible(fieldGroupName='dialogWidgetPramsFg',
                                            fieldNames=['dataField', 'recordsetField', 'dataUrlField', 'valueField', 'displayField', 'serverFilterField', 'pageSizeField'],
                                            visible=True)
                ...
                elif widgetNm == ikui.SCREEN_FIELD_WIDGET_INLINE_RADIO_GROUP:
                    screen.setFieldsVisible(fieldGroupName='dialogWidgetPramsFg',
                                            fieldNames=['multipleField', 'dataField', 'recordsetField', 'dataUrlField', 'valueField', 'displayField', 'columnField'],
                                            visible=True)
            ...
            isSelectedScreenSn = True if not str_utils.isEmpty(selectedScreenSn) else False
    ...
    def getScreenSelectRc(self):
        data = None
        if not str_utils.isEmpty(self.getSessionParameter("screenSN")):
    ...
    def getScreenRc(self):
        ...
        if not str_utils.isEmpty(screenSn) and not isNewScreen:
    ...
    def saveScreen(self):
        screenSn = self.getSessionParameter("screenSN")
        isNewScreen = self.getSessionParameterBool("isNewScreen")
        if str_utils.isEmpty(screenSn) and not isNewScreen:
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.saveScreen(self, screenSn, isNewScreen, True)
        if boo.value:
            if isNewScreen:
    ...
    def deleteScreen(self):
        ...
        if str_utils.isEmpty(screenSn) and not isNewScreen:
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.deleteScreen(self, screenSn)
        if boo.value:
            self.deleteSessionParameters(
                nameFilters=['isNewScreen', 'screenSN', 'isNewFg', 'currentFgID', 'isNewFgLink', 'currentFgLinkID', 'isNewFgHeaderFooter', 'currentFgHeaderFooterID'])
        return boo.toIkJsonResponse1()
    ...
    def deleteLastScreen(self):
        ...
        if str_utils.isEmpty(screenSn) and not isNewScreen:
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.deleteLastScreen(self, screenSn)
        if boo.value:
    ...
    def copyScreen(self):
        ...
        boo = screen_dfn_manager.copyScreen(self, userID, screenSn, newScreenSn)
        return boo.toIkJsonResponse1()
    ...
    def resetRev(self):
        ...
        b = screen_dfn_manager.resetRev(userID, screenSn, newRev)
        return b.toIkJsonResponse1()
    ...
    def getRecordsetRcs(self):
        recordsetRcs = []
        screenSn = self.getSessionParameter("screenSN")
        if not str_utils.isEmpty(screenSn):
            screenRc = Screen.objects.filter(screen_sn__iexact=screenSn).order_by("-rev").first()
    ...
    def saveRecordsets(self):
        ...
        if str_utils.isEmpty(screenSn):
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.saveScreenRecordsets(self, screenSn, True)
        if boo.value:
            return IkSccJsonResponse(message="Saved.")
        return boo.toIkJsonResponse1()
    ...
    def fieldGroupListFg_EditIndexField_Click(self):
        fieldGroupId = self._getEditIndexField()
        fieldGroupId = fieldGroupId if not str_utils.isEmpty(fieldGroupId) else self.getSessionParameterInt("currentFgID")
        if str_utils.isEmpty(fieldGroupId):
    ...
    def newFieldGroup(self):
        self._deleteEditIndexFieldValue(fieldGroupName='fieldGroupListFg')
        self.deleteSessionParameters(nameFilters=['isNewScreen', 'isNewFg', 'currentFgID', 'isNewFgLink', 'currentFgLinkID', 'isNewFgHeaderFooter', 'currentFgHeaderFooterID'])
    ...
    def saveFieldGroup(self):
        ...
        if str_utils.isEmpty(screenSn):
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.saveFieldGroup(self, screenSn, isNewFg, True)
        if boo.value:
            currentFgId = self.getSessionParameter('currentFgID')
            self._setEditIndexFieldValue(currentFgId, fieldGroupName='fieldGroupListFg')
            self.deleteSessionParameters("isNewFg")
            return IkSccJsonResponse(message="Saved.")
        return boo.toIkJsonResponse1()
    ...
    def deleteFieldGroup(self):
        ...
        boo = screen_dfn_manager.deleteFieldGroup(self, screenSn, fieldGroupId)
    ...
    def saveSubScreen(self):
        ...
        if str_utils.isEmpty(screenSn):
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.saveSubScreen(self, screenSn, True)
        if boo.value:
            currentFgId = self.getSessionParameter('currentFgID')
            self._setEditIndexFieldValue(currentFgId, fieldGroupName='fieldGroupListFg')
            return IkSccJsonResponse(message="Saved.")
    ...
    def fgLinkListFg_EditIndexField_Click(self):
        fgLinkId = self._getEditIndexField()
        fgLinkId = fgLinkId if not str_utils.isEmpty(fgLinkId) else self.getSessionParameterInt("currentFgLinkID")
        if str_utils.isEmpty(fgLinkId):
    ...
    def changeDbKeyAndPFgs(self):
        ...
        # db keys
        dbKeyData = screen_dfn_manager.getFieldDBKeys(screenRc, ScreenFieldGroup.objects.filter(id=fgId).first())
    ...
    def getFgFieldDbKeys(self):
        ...
        if fgLinkId:
            if isNewFgLink:
                ...
                data = screen_dfn_manager.getFieldDBKeys(screenRc, field_group)
            else:
    ...
    def getParentFgFieldDbKeys(self):
        ...
        if fgLinkId:
            if isNewFgLink:
                ...
                data = screen_dfn_manager.getFieldDBKeys(screenRc, parent_field_group)
        return data
    ...
    # events
    def newFgLink(self):
        self._deleteEditIndexFieldValue(fieldGroupName='fgLinkListFg')
        self.deleteSessionParameters(nameFilters=['isNewScreen', 'isNewFg', 'currentFgID', 'isNewFgLink', 'currentFgLinkID', 'isNewFgHeaderFooter', 'currentFgHeaderFooterID'])
    ...
    def saveFgLink(self):
        ...
        if str_utils.isEmpty(screenSn):
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.saveFgLink(self, screenSn, isNewFgLink, True)
        if boo.value:
            if isNewFgLink:
                self.deleteSessionParameters("isNewFgLink")
            currentFgLinkID = self.getSessionParameter("currentFgLinkID")
            self._setEditIndexFieldValue(currentFgLinkID, fieldGroupName='fgLinkListFg')
            return IkSccJsonResponse(message='Saved.')
        return boo.toIkJsonResponse1()
    ...
    def deleteFgLink(self):
        ...
        boo = screen_dfn_manager.deleteFgLink(self, screenSn, fgLinkId)
    ...
    def fgHeaderFooterListFg_EditIndexField_Click(self):
        fgHeaderFooterId = self._getEditIndexField()
        fgHeaderFooterId = fgHeaderFooterId if not str_utils.isEmpty(fgHeaderFooterId) else self.getSessionParameterInt("currentFgHeaderFooterID")
        if str_utils.isEmpty(fgHeaderFooterId):
    ...
    def newFgHeaderFooter(self):
        self._deleteEditIndexFieldValue(fieldGroupName='fgHeaderFooterListFg')
        self.deleteSessionParameters(nameFilters=['isNewScreen', 'isNewFg', 'currentFgID', 'isNewFgLink', 'currentFgLinkID', 'isNewFgHeaderFooter', 'currentFgHeaderFooterID'])
    ...
    def saveFgHeaderFooter(self):
        ...
        if str_utils.isEmpty(screenSn):
            return IkErrJsonResponse(message="Please select a Screen first.")
        boo = screen_dfn_manager.saveFgHeaderFooter(self, screenSn, isNewFgHeaderFooter, True)
        if boo.value:
            if isNewFgHeaderFooter:
                self.deleteSessionParameters("isNewFgHeaderFooter")
            fgHeaderFooterId = self.getSessionParameter("currentFgHeaderFooterID")
            self._setEditIndexFieldValue(fgHeaderFooterId, fieldGroupName='fgHeaderFooterListFg')
        return boo.toIkJsonResponse1()
    ...
    def deleteFgHeaderFooter(self):
        ...
        boo = screen_dfn_manager.deleteFgHeaderFooter(self, screenSn, fgHeaderFooterId)
    ...
    def postRow(self):
        ...
        if isNullBlank(widgetID):
            ...
            if isNotNullBlank(widgetRc):
                ...
                if widgetRc.widget_nm in screen_dfn_manager.NO_PARAMETERS_WIDGET:
    ...
    def uploadWidgetPrams(self):
        ...
        if isNotNullBlank(widgetPrams):
            ...
            if 'recordset' in cleanedDict and isNotNullBlank(cleanedDict['recordset']):
                ...
                sql_models = recordsetRc.sql_models if recordsetRc else ''
                if "." not in sql_models:
                    screenSn = self.getSessionParameter('screenSN')
                    screenRc = Screen.objects.filter(screen_sn__iexact=screenSn).order_by("-rev").first()
                    sql_models = "%s.models.%s" % (screenRc.app_nm, sql_models) if screenRc else ''
                recordsetClass = model_utils.getModelClass(sql_models)
                if 'DummyModel' not in sql_models:
            ...
            if 'server_filter' in cleanedDict and cleanedDict['server_filter'] == 'false':
                cleanedDict.pop('server_filter')
