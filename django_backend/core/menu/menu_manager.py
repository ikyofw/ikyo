...
class _MenuManager():
    ...
    def build_menu_index(self) -> Dict[int, dict]:
        rows = ikModels.Menu.objects.values('id', 'menu_nm', 'menu_caption', 'parent_menu_id')
        return {r['id']: r for r in rows}

    def get_full_menu_name(self, menu_id: int) -> str:
        menu_index = self.build_menu_index()
        node = menu_index.get(menu_id)
        if not node:
            raise IkValidateException(f'Menu ID[{menu_id}] is not found.')

        # Safety check: prevent cyclic ancestors (invalid data)
        visited = set()
        parts = [f"{node['menu_nm']} - {node['menu_caption']}"]
        ...
        while parent_id:
            ...
            parent = menu_index.get(parent_id)
            if not parent:
                # Missing parent node, stop traversal
                break
            parts.insert(0, f"{parent['menu_caption']}")
            parent_id = parent['parent_menu_id']

        # For parent menus, only display the caption;
        # for the current menu, display “nm - caption”
        # Original logic: “Parent1 -> Parent2 -> Child nm - Child caption”
        return " -> ".join(parts)
