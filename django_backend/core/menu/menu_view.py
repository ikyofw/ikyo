...
from core.sys.access_log import getAccessLog, getLatestAccessLog
...
class MenuMnt(ScreenAPIView):
    ...
    def get_menu_rcs(self):
        sch_items = self.getSessionParameter('sch_items')
        menu_rcs = menuMntManager.get_menu_tree(sch_items)
        return self.getPagingResponse(table_name="menuFg", table_data=menu_rcs)
