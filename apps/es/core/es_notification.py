...
def _send_notify_to_inbox(sender_id: int, receiver_id: int, category: MessageCategory, status: str, summary: str, id: int, menu_id: int, add_url: bool) -> Boolean2:
    ...
    params = {}
    if add_url:
        params.update({inbox_manager.ACTION_COMMAND: menu_id})
        if isNotNullBlank(id) and int(id) > 0:
            params.update({"id": id})
    success_msgs = []
    failed_msgs = []
    try:
        inbox_manager.send(senderID=sender_id, receiverIDs=receiver_id, module=category.value, summary=summary, linkParameters=params)
