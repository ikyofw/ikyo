...
from ..models import (Activity, AvailablePriorBalance, CashAdvancement,
                      ...
                      Po, PriorBalance)
from . import acl, approver, es, es_file, es_notification, es_seq, es_tools, po
...
def submit_cash_advancement(claimer_id: int, cash_advancement_id: int, office_rc: Office, ccy_rc: Currency, payee_rc: Payee,
    ...
    advance_amount = es_tools.round2(float(advance_amount))
    if advance_amount <= 0:
    ...
    try:
        ...
        if is_new:
            ...
            ca_rc.sn = es_seq.getNextSN(es_seq.SequenceType.SEQ_TYPE_CASH_ADVANCEMENT_SN, office_rc.id)
        ...
        try:
            if not is_success:
                if is_new and isNotNullBlank(ca_rc) and isNotNullBlank(ca_rc.sn):
                    logger.info('Rollback office [%s] sequence [%s][%s].' % (office_rc.code, es_seq.SequenceType.SEQ_TYPE_CASH_ADVANCEMENT_SN.value, ca_rc.sn))
                    es_seq.rollbackSeq(es_seq.SequenceType.SEQ_TYPE_CASH_ADVANCEMENT_SN, office_rc, es_seq.SN2Number(ca_rc.sn))
        except Exception as e:
            logger.error("Rollback cash advancement sequence [%s] failed: %s" % (ca_rc.sn, str(e)), e, exc_info=True)
        __CA_NEW_LOCK.release()
        if is_success:
            es_notification.send_submit_cancel_reject_cash_advancement_notify(claimer_id, ca_rc)
...
def cancel_cash_advancement(operator_id: int, cash_advancement_id: int, cancel_reason: str) -> Boolean2:
    ...
    try:
        ...
        if is_success:
            es_notification.send_submit_cancel_reject_cash_advancement_notify(operator_id, ca_rc)
...
def approve_cash_advancement(operator_id: int, cash_advancement_id: int) -> Boolean2:
    ...
    try:
        ...
        __CA_OPERATION_LOCK.release()
        if is_success:
            es_notification.send_approve_cash_advancement_notify(operator_id, ca_rc)
    ...
    try:
        ...
        if isNotNullBlank(payment_record_file):
            payment_record_file_rc = es.prepare_upload_file(
                ca_rc.office, es_file.FileCategory.PAYMENT_RECORD, payment_record_file)
                ...
                if isNotNullBlank(payment_record_file_seq):  # rollback file seq
                    try:
                        newSeq = es_seq.rollbackSeq(
                            es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE, ca_rc.office, payment_record_file_seq, exact=True)
                        ...
                        logger.error('Rollback file [%s] sequence [%s] failed: %s' %
                                     (es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE.value, payment_record_file_seq, str(e)), e, exc_info=True)
                if isNotNullBlank(payment_record_file_id):
                    try:
                        if not es_file.rollbackFileRecord(operator_id=operator_id, file_id=payment_record_file_id, file_rc=payment_record_file_rc):
                            logger.error(
...
def revert_settled_cash_advancement(operator_id: int, cash_advancement_id: int, reason: str) -> Boolean2:
    ...
    try:
        ...
        if isNotNullBlank(payment_record_file_rc):
            payment_record_file_id, payment_record_file_seq = payment_record_file_rc.id, payment_record_file_rc.seq
            payment_record_file_path = es_file.getReallyFile(payment_record_file_rc)
        ...
        try:
            if is_success:
                ...
                if isNotNullBlank(payment_record_file_seq):  # rollback file seq
                    try:
                        es_seq.rollbackSeq(
                            es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE, ca_rc.office, payment_record_file_seq)
                    except Exception as e:
                        logger.error('Rollback file [%s] sequence [%s] failed: %s' %
                                     (es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE.value, payment_record_file_seq, str(e)), e, exc_info=True)
                if isNotNullBlank(payment_record_file_path):
                    es_file.deleteESFileAndFolder(payment_record_file_path)
        except Exception as e:
            logger.error('Rollback the uploaded payment record file failed. %s' % str(
                e), e, exc_info=True)
    ...
    if cashAdvRc.sts == Status.SETTLED.value:
        ...
        for pbRc in PriorBalance.objects.filter(ca=cashAdvRc, expense__sts__in=__CASH_ADVANCEMENT_ACCEPT_EXPENSE_STATUS):
            if isNotNullBlank(pbRc.fx):
                ...
                fxUsedAmount = es_tools.add(fxUsedAmount, pbRc.fx_balance_amt)
                ...
                for fxData in fxUsages:
                    if fxData[0].id == pbRc.fx.id:
                        fxUsed = float(round_currency(es_tools.add(fxData[2], pbRc.fx_balance_amt)))
                        fxLeft = float(round_currency(es_tools.sub(fxData[1], fxUsed)))
                        fxData[2] = fxUsed
                        fxData[3] = fxLeft
                        break
            else:
                usedLocalAmount = es_tools.add(usedLocalAmount, pbRc.balance_amt)
        ...
        usages.append((cashAdvRc.ccy, False, float(round_currency(localTotalAmount)),
                       ...
                       float(round_currency(es_tools.sub(localTotalAmount, usedLocalAmount)))))
        for fxID, fxAmount in fxAmounts.items():
            fxUsedAmount = fxUsedAmounts.get(fxID, 0)
            fxLeft = float(round_currency(
                es_tools.sub(fxAmount, fxUsedAmount)))
    ...
    for pbRc in pbRcs:
        ...
        if isNotNullBlank(pbRc.fx):
            ...
            if isNullBlank(totalFxUsedAmount):
                totalFxUsedAmount = 0
            amount = float(round_currency(es_tools.sub(pbRc.ca.claim_amt, totalFxUsedAmount)))
        ...
        leftAmount = float(round_currency(es_tools.sub(es_tools.sub(amount, previousExpenditureAmount if isNotNullBlank(previousExpenditureAmount) else 0), claimAmount)))
