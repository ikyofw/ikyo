...
from ..models import *
from . import (acl, ca, es_file, es_notification, es_seq, es_tools,
               petty_expense, po)
...
def getExpensePriorBalanceAmount(hdrID: int) -> float:
    ...
    amount = 0
    for r in pbRcs:
        amount = round_currency(es_tools.add(amount, r.balance_amt))
...
def __create_draft_expense(claimer_rc: User, office_rc: Office) -> Expense:
    ...
    if expense_rc is None:
        expense_rc = Expense()
        expense_rc.sn = es_seq.getDraftSN(office_rc)
...
def prepare_upload_file(office_rc: Office, file_category: es_file.FileCategory, file: Path, specified_file_seq: int = None) -> File:
    seq_type = None
    if file_category == es_file.FileCategory.PAYMENT_RECORD:
        seq_type = es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE
    elif file_category == es_file.FileCategory.INVOICE:
        seq_type = es_seq.SequenceType.SEQ_TYPE_EXPENSE_FILE
    elif file_category == es_file.FileCategory.EXCHANGE_RATE_RECEIPT:
        seq_type = es_seq.SequenceType.SEQ_TYPE_EXCHANGE_RECEIPT_FILE
    elif file_category == es_file.FileCategory.SUPPORTING_DOCUMENT:
        seq_type = es_seq.SequenceType.SEQ_TYPE_SUPPORTING_DOCUMENT
    elif file_category == es_file.FileCategory.PO:
        seq_type = es_seq.SequenceType.SEQ_TYPE_PO_FILE
    else:
    ...
    if isNotNullBlank(specified_file_seq):
        ...
        file_seq = es_seq.getNextSeq(seq_type, office_rc.id)
    file_rc = File()
    ...
    file_absolute_path = es_file.getUploadFileAbsolutePath(relative_path_str, save_file_name)
    ...
    es_file.delete_really_file(file)  # delete empty folder
    ...
    file_rc.sha256 = es_file.calculateFileHash(file_absolute_path)
    return file_rc
...
def upload_expense_file(claimer_rc: User,
    ...
    if not es_file.validateUploadFileType(uploaded_file_path):
        raise IkValidateException('UnSupport file [%s]. Only %s allowed.' % (uploaded_file_path, es_file.ALLOW_FILE_TYPES))
    ...
    try:
        ...
        new_file_rc = prepare_upload_file(office_rc, es_file.FileCategory.INVOICE, uploaded_file_path, original_file_seq)
            ...
            if is_success:
                try:
                    if delete_original_file_info is not None:
                        # delete the old file
                        originalFile = es_file.getIdFile(delete_original_file_info[0], delete_original_file_info[1])
                ...
                if new_file_rc is not None:
                    if isNotNullBlank(new_file_rc.seq):  # rollback file seq
                        try:
                            es_seq.rollbackSeq(es_seq.SequenceType.SEQ_TYPE_EXPENSE_FILE, new_file_rc.office.id, new_file_rc.seq)
                        except Exception as e:
                            logger.error('Rollback file [%s] sequence [%s] failed: %s' %
                                         (es_seq.SequenceType.SEQ_TYPE_EXPENSE_FILE.value, new_file_rc.seq, str(e)))
                            logger.error(e, exc_info=True)
                    if isNotNullBlank(new_file_rc.id):
                        try:
                            if not es_file.rollbackFileRecord(claimer_rc.id, new_file_rc.id):
...
def delete_uploaded_expense_file(claimer_rc: User,
    ...
    try:
        ...
        is_need_to_rollback_file_seq = (file_rc.seq == es_seq.getCurrentSeq(es_seq.SequenceType.SEQ_TYPE_EXPENSE_FILE, office_rc.id))
        ...
        if is_need_to_rollback_file_seq:
            es_seq.rollbackSeq(es_seq.SequenceType.SEQ_TYPE_EXPENSE_FILE, office_rc.id, file_rc.seq, transaction=trn)
            ...
            if is_success:
                try:
                    if delete_file_info is not None:
                        # delete the old file
                        old_file = es_file.getIdFile(delete_file_info[0], delete_file_info[1])
...
def __delete_file_from_file_system(file_path: Path) -> None:
    es_file.deleteESFileAndFolder(file_path)
...
def __getTotalAmount(expenseDetailRcs: list[ExpenseDetail], ccy: Currency) -> Decimal:
    ...
    for rc in expenseDetailRcs:
        if rc.ik_is_status_delete():
            continue
        if rc.ccy.id == ccy.id:
            totalAmount = es_tools.add(totalAmount, rc.amt)
        else:
            if isNotNullBlank(rc.ex_rate):
                totalAmount = es_tools.add(totalAmount, round_currency(es_tools.mul(rc.amt, rc.ex_rate)))
            else:
                totalAmount = es_tools.add(totalAmount, rc.amt)
    return totalAmount
...
def getInvoiceAmount(localCCY: str, fileID: int) -> Decimal:
    ...
    for rc in ExpenseDetail.objects.filter(file=fileRc).all():
        if rc.ccy == localCCY:
            totalAmount = es_tools.add(Decimal(totalAmount), rc.amt)
        else:
            ex_rate = 1 if isNullBlank(rc.ex_rate) else rc.ex_rate
            totalAmount = es_tools.add(Decimal(totalAmount), Decimal(round_currency(rc.amt * ex_rate)))
...
def submitExpense(claimer_rc: User, office_rc: Office, expense_id: int, payeeID: int, approverID: int,
    ...
    try:
        ...
        for expenseRc in expense_dtl_rcs:
            ...
            ccyRc = expenseRc.ccy
            # expense amount can be less than 0 - 20251125
            #if expenseRc.amt <= 0:
            ...
            if ccyRc.id == payeeDefaultCCYRc.id:
                totalExpenseClaimAmount = es_tools.add(totalExpenseClaimAmount, expenseRc.amt)
            else:
                totalExpenseClaimAmount = es_tools.add(totalExpenseClaimAmount, round_currency(expenseRc.amt * exRate))
        ...
        if priorBalanceRcs and len(priorBalanceRcs) > 0:
            ...
            if isSettleByPriorBalance:
                ...
                for pbRc in priorBalanceRcs:
                    ...
                    elif es_tools.isGreater(thisClaim, pbRc.balance_amt):
                    ...
                    if not availableValidate:
                        raise IkValidateException("Concurrency error, please refresh the screen to try again.")
                    priorBalancePayAmount = es_tools.add(priorBalancePayAmount, thisClaim)

                    # prepare data object for saving
                    pbNewRc = PriorBalance(ca=pbRc.ca, expense=hdr_rc)
                    if isSettleByFXCash:
                        pbNewRc.balance_amt = round_currency(es_tools.div(pbRc.deduction_amt, pbRc.fx.fx_rate))
                        pbNewRc.fx_balance_amt = pbRc.deduction_amt
                        pbNewRc.fx = pbRc.fx
                    else:
                        pbNewRc.balance_amt = pbRc.deduction_amt
                    priorBalanceNewRcs.append(pbNewRc)
                    totalDefaultCCYAmount = es_tools.add(totalDefaultCCYAmount, pbNewRc.balance_amt)
            ...
            thisPayAmount = float(round_currency(es_tools.sub(totalExpenseClaimAmount, priorBalancePayAmount)))
        ...
        if isDraft:
            newExpenseSN = es_seq.getNextSN(es_seq.SequenceType.SEQ_TYPE_EXPENSE_SN, office_rc.id)
        ...
        if isSubmitSuccess:
            es_notification.send_submit_cancel_reject_expense_notify(claimer_rc.id, hdr_rc)
        else:
            if isNotNullBlank(newExpenseSN):
                try:
                    es_seq.rollbackSeq(es_seq.SequenceType.SEQ_TYPE_EXPENSE_SN, office_rc.code, es_seq.SN2Number(newExpenseSN))
                except Exception as e:
                    logger.error(e, exc_info=True)
...
def approve_expense(operator_id: int, expense_hdr_id: int) -> Boolean2:
    ...
    try:
        ...
        __EXPENSE_LOCK.release()
        if is_success:
            es_notification.send_approve_confirm_petty_expense_notify(operator_id, hdr_rc)
...
def cancel_expense(operator_id: int, expense_hdr_id: int, cancel_reason: str) -> Boolean2:
    ...
    try:
        ...
        __EXPENSE_LOCK.release()
        if is_success:
            es_notification.send_submit_cancel_reject_expense_notify(operator_id, hdr_rc)
...
def settle_expense(operator_id: int, expense_id: int, payment_tp: PaymentMethod, payment_number: str, payment_record_file: Path, payment_rmk: str) -> Boolean2:
    ...
    if isNotNullBlank(payment_record_file):
        ...
        elif not es_file.validateUploadFileType(payment_record_file):
            return Boolean2.FALSE('UnSupport file [%s]. Only %s allowed.' % (payment_record_file.name, es_file.ALLOW_FILE_TYPES))
    ...
    try:
        ...
        if isNotNullBlank(payment_record_file):
            payment_record_file_rc = prepare_upload_file(
                hdr_rc.office, es_file.FileCategory.PAYMENT_RECORD, payment_record_file)
        ...
        try:
            if not is_success:
                if isNotNullBlank(payment_record_fileSeq):  # rollback file seq
                    try:
                        newSeq = es_seq.rollbackSeq(
                            es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE, hdr_rc.office, payment_record_fileSeq, exact=True)
                        ...
                        logger.error('Rollback file [%s] sequence [%s] failed: %s' %
                                     (es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE.value, payment_record_fileSeq, str(e)), e, exc_info=True)
                if isNotNullBlank(payment_record_fileID):
                    try:
                        if not es_file.rollbackFileRecord(operator_id, payment_record_fileID):
    ...
    try:
        ...
        try:
            if is_success:
                ...
                if isNotNullBlank(payment_record_fileSeq):  # rollback file seq
                    try:
                        es_seq.rollbackSeq(es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE, hdr_rc.office, payment_record_fileSeq)
                    except Exception as e:
                        logger.error('Rollback file [%s] sequence [%s] failed: %s' %
                                     (es_seq.SequenceType.SEQ_TYPE_PAYMENT_RECORD_FILE.value, payment_record_fileSeq, str(e)), e, exc_info=True)
                if isNotNullBlank(payment_record_fileID):
...
def __validateExpensePriorBalance(hdrRc: Expense) -> None:
    ...
    pbAmount = 0
    for pbRc in pbRcs:
        pbAmount = es_tools.round2(es_tools.add(pbAmount, pbRc.claim_amt))
    if abs(float(es_tools.sub(hdrRc.claim_amt, es_tools.add(pbAmount, hdrRc.pay_amt)))) > 0.000001:
...
def uploadExpenseSupportingDocument(operatorID: int, expense_rc: Expense, temp_upload_file: Path) -> tuple:
    ...
    new_file_sha256 = es_file.calculateFileHash(temp_upload_file)
    ...
    try:
        ...
        new_file_rc = prepare_upload_file(expense_rc.office, es_file.FileCategory.SUPPORTING_DOCUMENT,
            ...
            if not is_success:
                if isNotNullBlank(new_file_rc):
                    if isNotNullBlank(new_file_rc.seq) and not ((is_draft and isNotNullBlank(old_file_rc))):  # rollback file seq
                        try:
                            es_seq.rollbackSeq(es_seq.SequenceType.SEQ_TYPE_SUPPORTING_DOCUMENT, expense_rc.office, new_file_rc.seq)
                        except Exception as e:
                            logger.error('Rollback file [%s] sequence [%s] failed: %s' %
                                         (es_seq.SequenceType.SEQ_TYPE_SUPPORTING_DOCUMENT.value, new_file_rc.seq, str(e)), e, exc_info=True)
                    if isNotNullBlank(new_file_rc.id):
                        try:
                            if not es_file.rollbackFileRecord(operatorID, None, new_file_rc):
                ...
                if isNotNullBlank(old_file_rc):
                    try:
                        if not es_file.rollbackFileRecord(operatorID, None, old_file_rc):
...
def deleteExpenseSupportingDocument(operatorID: int, expenseHdrRc: Expense) -> int:
    ...
    try:
        ...
        supporting_doc_file_path = es_file.getReallyFile(uploadFileRc)
        ...
        try:
            if is_success:
                if isNotNullBlank(fileSeq):  # rollback file seq
                    try:
                        es_seq.rollbackSeq(es_seq.SequenceType.SEQ_TYPE_SUPPORTING_DOCUMENT, expenseHdrRc.office, fileSeq)
                    except Exception as e:
                        logger.error('Rollback file [%s] sequence [%s] failed: %s' %
                                     (es_seq.SequenceType.SEQ_TYPE_SUPPORTING_DOCUMENT.value, fileSeq, str(e)), e, exc_info=True)
                if isNotNullBlank(fileID):
                    try:
                        if not es_file.rollbackFileRecord(operatorID, fileID):
                    ...
                    try:
                        # delete the file
                        es_file.deleteESFileAndFolder(supporting_doc_file_path)
                    except Exception as e:
                        logger.error('Delete supporting document [%s] failed: %s' % (fileID, str(e)), e, exc_info=True)
        except:
...
def getExpenseSupportingDocumentFilename(fileRc: File) -> str:
    f = es_file.getFile(fileRc.id)
    fileType = Path(f).suffix[1:]
    sn = es_seq.getSupportingDocumentFileSN(fileRc.seq)
